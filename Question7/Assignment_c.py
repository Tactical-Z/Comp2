...
sol_NonJ2 = solve_ivp(eom, (0,t_final), state_0, t_eval=time, rtol=1e-9, args=(False,))
sol_J2 = solve_ivp(eom, (0,t_final), state_0, t_eval=time, rtol=1e-9, args=(True,))
...
eci_coordsNonJ2 = sol_NonJ2.y[:3, :]  # Get first 3 rows (position)
ecef_coordsNonJ2 = R_ECI2ECEF @ eci_coordsNonJ2 # apply rotation to all points
x_ecefNonJ2, y_ecefNonJ2, z_ecefNonJ2 = ecef_coordsNonJ2 # make individual components for chart
...
ax.set_ylabel("y (km)")
ax.set_zlabel("z (km)")
ax.set_title("Satellite Trajectory in ECI Frame (100 Orbits) including J2 pertubations")
...
ax.add_collection(lc_2dJ2)
ax.set_xlabel("ECEF X (km)")
ax.set_ylabel("ECEF Y (km)")
ax.set_title("Satellite Orbit in ECEF Frame including J2 pertubations")
ax.set_aspect('equal')
ax.grid(True)
lim = R + 3000 # margine lim a little larger 
ax.set_xlim(-lim, lim)
ax.set_ylim(-lim, lim)
plt.show()
# ------------------------------ J2 2D Plot ------------------------------
...
# Non J2 pertubation plot cordinator
points_2dNonJ2 = np.array([x_ecefNonJ2, y_ecefNonJ2]).T.reshape(-1, 1, 2)
segments_2dNonJ2 = np.concatenate([points_2dNonJ2[:-1], points_2dNonJ2[1:]], axis=1)
normNonJ2 = plt.Normalize(0, len(time) - 1)
colorsNonJ2 = cm.viridis(normNonJ2(range(len(time)-1)))
lc_2dNonJ2 = mcoll.LineCollection(segments_2dNonJ2, colors=colorsNonJ2, linewidths=2)
...
# Chart orbit over color map rather than solid color 
# -----------------
points = np.array([sol_NonJ2.y[0], sol_NonJ2.y[1], sol_NonJ2.y[2]]).T.reshape(-1,1,3)
segments = np.concatenate([points[:-1], points[1:]], axis=1)
norm = plt.Normalize(0, len(sol_NonJ2.t))
colors = cm.viridis(norm(range(len(sol_NonJ2.t)-1)))
lc = art3d.Line3DCollection(segments, colors=colors, linewidths=2)
ax.add_collection(lc)
# -----------------
ax.plot_surface(x_earth, y_earth, z_earth, color='b', alpha=0.2)
...
ax.set_ylabel("y (km)")
ax.set_zlabel("z (km)")
ax.set_title("Satellite Trajectory in ECI Frame (100 Orbits) excluding J2 pertubations")
ax.set_box_aspect([1,1,1])
plt.grid(True)
plt.show()
# ------------------------------ NON J2 3D Plot ------------------------------
...
# ------------------------------ NON J2 2D Plot ------------------------------
earth_circle = plt.Circle((0, 0), R, color='b', alpha=0.2) # Earth radius for reference
fig, ax = plt.subplots(figsize=(8,8))
ax.add_artist(earth_circle)
ax.add_collection(lc_2dNonJ2)
ax.set_xlabel("ECEF X (km)")
ax.set_ylabel("ECEF Y (km)")
ax.set_title("Satellite Orbit in ECEF Frame excluding J2 pertubations")
ax.set_aspect('equal')
ax.grid(True)
lim = R + 3000 # margine lim a little larger 
ax.set_xlim(-lim, lim)
